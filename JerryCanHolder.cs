...
namespace TommoJProductions.JerryCanHolderMod
{
    public class JerryCanHolder : Mod
    {
        ...
        private FsmState openHoodState;
        ...
        private void load()
        {
            ...
            if (tryLoadAssets()) 
            {
                ...
                FsmState openHoodState = p.GetState("Open hood");
                openHoodState.insertNewAction(calTorqueTransition, 3, CallbackTypeEnum.onFixedUpdate, true);
                openHoodState.RemoveAction(4); // remove addTorque action
            }
        }
